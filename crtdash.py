...
refresh_rate = st.sidebar.selectbox("Refresh Rate (seconds)", [5, 10, 30, 60], index=1)

# Fetch and process data
if 'data' not in st.session_state or 'last_updated' not in st.session_state:
    st.session_state.data = fetch_data()
    st.session_state.last_updated = datetime.now()

current_time = datetime.now()
time_diff = (current_time - st.session_state.last_updated).total_seconds()
